...
@Component({
  ...
  template: `
    ...
    <ion-content>
      ...
      <ion-button (click)="incrementSuccessNb()">increment successNb</ion-button>
    </ion-content>
  `
...
export class IdbComponent {
  ...
  constructor() {
    // this.create(2,3,[{id: '1x1', successes: 2, fails: 2}])
    this.getSuccessNb().then(val => this.successNb = val)
  }

  async create(successNb: number, timeInterval: number, multiples: IMultiple[]) {
    ...
    multiples.forEach(m => {
      const {id, ...data} = m;
      multiplesObj[id] = data;
    })
    tx.store.put(multiplesObj, 'multiples')
    tx.done.then(console.log)
  }

  async getSuccessNb(): Promise<number> {
    const db = await this.db;
    return db.get(STORE_NAME, 'successNb');
  }

  async storeSuccessNb(nb: number) {
    const db = await this.db;
    db.put(STORE_NAME, nb, 'successNb');
  }

  async incrementSuccessNb() {
    if (this.successNb) {
      this.successNb += 1;
      this.storeSuccessNb(this.successNb);
    }
  }

  async addSuccess(multipleId: string) {
    const db = await this.db
    const multiplesDatas: IMultipleDatas = await db.get(STORE_NAME, 'multiples');
    multiplesDatas[multipleId].successes += 1;
    db.put('localTraining', multiplesDatas, 'multiples')
  }
