...
export class TestService {
  ...
  getLastUncompletedTest$(): Observable<Test | null> {
    const user$ = this.auth.getUser$$().pipe(take(1));
    const uncompleteTest$ = (user: User | null): Observable<Test | null> => {
      if (user) {
        const coll = this.afStore.collection<ITest>(`users/${user.uid}/tests`, ref => ref.orderBy('creationDate', 'desc').where('completed', '==', true));
    ...
    return user$.pipe(
      switchMap(usr => uncompleteTest$(usr))
    )
  }
